    ...
    ui(new Ui::Close)
{
    ...
    connect(m_pSpinBox, SIGNAL(valueChanged(int)), this, SLOT(ManualCloseChangeSlot(int)));
    connect(m_pSlider, SIGNAL(valueChanged(int)), this, SLOT(ManualCloseChangeSlot(int)));
    ...
    connect(button, SIGNAL(accepted()), parent, SLOT(OKCloseSliderSelectImg()));
    connect(button, SIGNAL(rejected()), parent, SLOT(CancelCloseSliderSelectImg()));
...
void Close::ManualCloseChangeSlot(int nValue)
{
    connect(this, SIGNAL(sendManualCloseChange(int , QString)), m_parent, SLOT(ManualCloseChangeSlot(int, QString)));
    emit sendManualCloseChange(nValue, m_StructureType);
    // 不加断开链接，执行第次越多，后面执行次数越多
    disconnect(this, 0, m_parent, 0);
}
void Close::SetStructureType(QString StructureType)
{
    m_StructureType = StructureType;
}
void Close::closeEvent(QCloseEvent *event)
{
    this->close();
}
